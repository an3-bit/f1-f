...
const QuestionFlow = () => {
  ...
  const handleAnswerSubmit = async (e) => {
    ...
    if (currentQuestionIndex + 1 < questions.length) {
      ...
      const payload = { ...answers, [questionKeyMap[currentQuestion]]: query.trim() };

      setProcessing(true);
      try {
        const res = await fetch("https://f1-backend-qhf8.onrender.com/api/triage/answers", {
          method: "POST",
        ...
        alert("Something went wrong while fetching the recommendation.");
      } finally {
        setProcessing(false);
      }
    }
  ...
  const handleSubmit = system => {
    navigate('/quote2', { state: { selectedSystem: system } });
  };
              ...
              {recommendation.recommended_systems.map((system, index) => (
                ...
                >
                  ...
                  <ul className="mt-3 space-y-1 text-sm text-gray-800">
                    <li>
                      <strong>Capacity:</strong> {system.specifications.tank_capacity}
                    </li>
                    <li>
                      <strong>Collector Type:</strong> {system.specifications.collector_type}
                    </li>
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    >
                      Choose this and proceed to quotation
                    </button>
                  </div>
                </div>
              ))}
